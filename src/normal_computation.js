...
export function compute_triangle_normals_and_angle_weights(mesh) {
	...
	*/
    ...
    for(let i_face = 0; i_face < num_faces; i_face++) {
        ...
        tri_normals.push(vec3.normalize([0, 0, 0], normal))
...
export function compute_vertex_normals(mesh, tri_normals, angle_weights) {
	...
	*/
    ...
    for(let i_face = 0; i_face < num_faces; i_face++) {
        ...
        vertex_normals[iv1] = vec3.add([0, 0, 0], vec3.scale([0, 0, 0], tri_normals[i_face], w1), vertex_normals[iv1])
        vertex_normals[iv2] = vec3.add([0, 0, 0], vec3.scale([0, 0, 0], tri_normals[i_face], w2), vertex_normals[iv2])
        vertex_normals[iv3] = vec3.add([0, 0, 0], vec3.scale([0, 0, 0], tri_normals[i_face], w3), vertex_normals[iv3])
    }
